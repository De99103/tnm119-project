...
# ==== Logistic Regression ====
model_LogRed = LogisticRegression(random_state= 10, penalty='l2', max_iter=5000 )
...
svm_clf = LinearSVC(C=4, random_state=seed)
...
# Get predicted labels and probabilities separately
Y_pred_SVM = svm_clf.predict(X_test_tfidf)  # For classification report and accuracy

# Use decision_function instead of predict_proba for ROC curve
y_prob_SVM = svm_clf.decision_function(X_test_tfidf) # For ROC curve
...
plt.plot(fpr_Rf, tpr_Rf, color='pink', label=f"Random Forest (AUC = {roc_auc_rf:.4f})")
plt.plot(fpr_SVM, tpr_SVM, color='darkgreen', label=f"SVM (AUC = {roc_auc_SVM:.4f})")
